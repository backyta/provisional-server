interface SeedDisciples {
  //* General Info
  firstName: string;
  lastName: string;
  gender: string;
  originCountry: string;
  birthDate: string | Date;
  age?: number;
  maritalStatus: string;
  numberChildren?: number;
  conversionDate?: string | Date;

  //* Contact Info
  email: string;
  phoneNumber: string;
  countryResidence?: string;
  departmentResidence?: string;
  provinceResidence?: string;
  districtResidence: string;
  urbanSectorResidence: string;
  addressResidence: string;
  addressResidenceReference: string;
  roles: string[];
  status?: string;

  //* Relations
  theirFamilyHouse?: string;
}

//? Array Interfaces
interface SeedDataMembers {
  disciples: SeedDisciples[];
}

//! Data Disciples
export const dataDisciples: SeedDataMembers = {
  disciples: [
    {
      firstName: 'Carlos',
      lastName: 'Gonzales Huaman',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '1990-12-05',
      maritalStatus: 'single',
      numberChildren: 3,
      conversionDate: '2008-07-20',
      email: 'carlos.gonzales@google.com',
      phoneNumber: '+1-567-8901',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Jr. Los Pinos 123',
      addressResidenceReference: 'Frente al parque zonal',
      roles: ['disciple'],
    },
    {
      firstName: 'Daniela',
      lastName: 'Rojas Gutierrez',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '1995-09-18',
      maritalStatus: 'single',
      numberChildren: 2,
      conversionDate: '2013-02-10',
      email: 'daniela.rojas@google.com',
      phoneNumber: '+1-890-1234',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Tahuantinsuyo',
      addressResidence: 'Av. Los Girasoles 456',
      addressResidenceReference: 'Cerca del mercado municipal',
      roles: ['disciple'],
    },
    {
      firstName: 'Mateo',
      lastName: 'Lopez Sanchez',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '2002-05-10',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2018-09-15',
      email: 'mateo.lopez@google.com',
      phoneNumber: '+1-234-5678',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Jr. Las Orquídeas 789',
      addressResidenceReference: 'A una cuadra del colegio',
      roles: ['disciple'],
    },
    {
      firstName: 'Valentina',
      lastName: 'Garcia Flores',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2007-11-30',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2022-02-18',
      email: 'valentina.garcia@google.com',
      phoneNumber: '+1-901-2345',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Payet',
      addressResidence: 'Av. Las Hortensias 890',
      addressResidenceReference: 'Frente al parque industrial',
      roles: ['disciple'],
    },
    {
      firstName: 'Pedro',
      lastName: 'Martinez Castillo',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '2010-03-15',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2025-01-20',
      email: 'pedro.martinez@google.com',
      phoneNumber: '+1-345-6789',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Tahuantinsuyo',
      addressResidence: 'Jr. Los Laureles 234',
      addressResidenceReference: 'Al lado del centro educativo',
      roles: ['disciple'],
    },
    {
      firstName: 'Ana',
      lastName: 'Torres Ramirez',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '1998-08-21',
      maritalStatus: 'single',
      numberChildren: 1,
      conversionDate: '2016-07-05',
      email: 'ana.torres@google.com',
      phoneNumber: '+1-567-8901',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Av. Los Jazmines 567',
      addressResidenceReference: 'Cerca del parque zonal',
      roles: ['disciple'],
    },
    {
      firstName: 'Diego',
      lastName: 'Alvarez Garcia',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '2004-12-10',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2020-05-15',
      email: 'diego.alvarez@google.com',
      phoneNumber: '+1-678-9012',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Payet',
      addressResidence: 'Jr. Las Acacias 890',
      addressResidenceReference: 'Frente al centro deportivo',
      roles: ['disciple'],
    },
    {
      firstName: 'Lucía',
      lastName: 'Rojas Gutierrez',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2001-04-25',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2019-10-20',
      email: 'lucia.rojas@google.com',
      phoneNumber: '+1-234-5678',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Tahuantinsuyo',
      addressResidence: 'Av. Las Rosas 123',
      addressResidenceReference: 'Frente a la plaza central',
      roles: ['disciple'],
    },
    {
      firstName: 'Eduardo',
      lastName: 'Martinez Castillo',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '2006-09-18',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2022-08-15',
      email: 'eduardo.martinez@google.com',
      phoneNumber: '+1-890-1234',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Jr. Los Pinos 456',
      addressResidenceReference: 'Al lado del parque zonal',
      roles: ['disciple'],
    },
    {
      firstName: 'Sofía',
      lastName: 'Gonzales Ramirez',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2009-07-30',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2024-12-20',
      email: 'sofia.gonzales@google.com',
      phoneNumber: '+1-901-2345',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Payet',
      addressResidence: 'Av. Los Nogales 890',
      addressResidenceReference: 'Frente al parque industrial',
      roles: ['disciple'],
    },
    {
      firstName: 'Javier',
      lastName: 'Fernandez Torres',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '2012-02-08',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2028-09-10',
      email: 'javier.fernandez@google.com',
      phoneNumber: '+1-567-8901',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Jr. Las Margaritas 678',
      addressResidenceReference: 'Frente al centro comercial',
      roles: ['disciple'],
    },
    {
      firstName: 'Camila',
      lastName: 'Martinez Garcia',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2015-05-20',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2031-11-15',
      email: 'camila.martinez@google.com',
      phoneNumber: '+1-234-5678',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Tahuantinsuyo',
      addressResidence: 'Av. Los Laureles 234',
      addressResidenceReference: 'Cerca del parque zonal',
      roles: ['disciple'],
    },
    {
      firstName: 'Juanito',
      lastName: 'Lopez Ramirez',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '1992-09-12',
      maritalStatus: 'single',
      numberChildren: 2,
      conversionDate: '2034-03-20',
      email: 'juanito.lopez@google.com',
      phoneNumber: '+1-890-1234',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Payet',
      addressResidence: 'Jr. Los Pinos 345',
      addressResidenceReference: 'Cerca del parque industrial',
      roles: ['disciple'],
    },
    {
      firstName: 'Valeria',
      lastName: 'Gomez Torres',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2000-11-18',
      maritalStatus: 'single',
      numberChildren: 1,
      conversionDate: '2029-07-05',
      email: 'valeria.gomez@google.com',
      phoneNumber: '+1-901-2345',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Av. Las Orquídeas 456',
      addressResidenceReference: 'Frente al centro de salud',
      roles: ['disciple'],
    },
    {
      firstName: 'Dieguito',
      lastName: 'Hernandez Garcia',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '1989-07-30',
      maritalStatus: 'single',
      numberChildren: 3,
      conversionDate: '2032-12-20',
      email: 'dieguito.hernandez@google.com',
      phoneNumber: '+1-345-6789',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Tahuantinsuyo',
      addressResidence: 'Jr. Las Rosas 567',
      addressResidenceReference: 'Cerca del mercado municipal',
      roles: ['disciple'],
    },
    {
      firstName: 'Laura',
      lastName: 'Perez Gutierrez',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2001-08-25',
      maritalStatus: 'single',
      numberChildren: 1,
      conversionDate: '2035-04-15',
      email: 'laura.perez@google.com',
      phoneNumber: '+1-678-9012',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Payet',
      addressResidence: 'Av. Las Acacias 678',
      addressResidenceReference: 'Frente al parque zonal',
      roles: ['disciple'],
    },
    {
      firstName: 'Pedrito',
      lastName: 'Gutierrez Martinez',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '1995-10-10',
      maritalStatus: 'single',
      numberChildren: 2,
      conversionDate: '2033-06-20',
      email: 'pedrito.gutierrez@google.com',
      phoneNumber: '+1-789-0123',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Jr. Los Nogales 789',
      addressResidenceReference: 'Frente al mercado municipal',
      roles: ['disciple'],
    },
    {
      firstName: 'Abril',
      lastName: 'Sanchez Diaz',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '1985-03-15',
      maritalStatus: 'single',
      numberChildren: 4,
      conversionDate: '2030-09-10',
      email: 'abril.sanchez@google.com',
      phoneNumber: '+1-012-3456',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Tahuantinsuyo',
      addressResidence: 'Av. Los Cedros 890',
      addressResidenceReference: 'Cerca del parque industrial',
      roles: ['disciple'],
    },
    {
      firstName: 'Marcos',
      lastName: 'Rodriguez Gutierrez',
      gender: 'male',
      originCountry: 'Peru',
      birthDate: '1988-12-05',
      maritalStatus: 'single',
      numberChildren: 5,
      conversionDate: '2036-07-20',
      email: 'marcos.rodriguez@google.com',
      phoneNumber: '+1-345-6789',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Payet',
      addressResidence: 'Jr. Las Margaritas 123',
      addressResidenceReference: 'Al lado del parque zonal',
      roles: ['disciple'],
    },
    {
      firstName: 'Alejandra',
      lastName: 'Martinez Flores',
      gender: 'female',
      originCountry: 'Peru',
      birthDate: '2000-11-20',
      maritalStatus: 'single',
      numberChildren: 0,
      conversionDate: '2027-02-15',
      email: 'alejandra.martinez@google.com',
      phoneNumber: '+1-678-9012',
      districtResidence: 'Independencia',
      urbanSectorResidence: 'Ermitaño',
      addressResidence: 'Av. Las Orquídeas 890',
      addressResidenceReference: 'Cerca del centro de salud',
      roles: ['disciple'],
    },
  ],
};
